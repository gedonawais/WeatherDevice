...
def check_sim_at(timeout=10):
    ...
    try:
        ...
        print("AT check failed:", e)
        log_no_time(f"AT check failed: {e}")
        return False
...
def set_cmnb_mode():
    try:
        ...
        print(f"Failed to set CMNB mode: {e}")
...
def reset_sim7070():
    ...
    try:
        ...
        print(f"Failed to send CFUN reset: {e}")
...
def checkConnection():
    try:
        ...
        if "+CSQ:" in raw:
            ...
            csq = int(value)
            if csq < 10:   status = "Bad, CSQ is <10"
            elif csq < 15: status = "Weak, CSQ is <15"
        ...
        log_no_time(f"checkConnection failed: {e}")
        return False
...
def init_connection():
    ...
    for attempt in range(MAX_PPP_RETRIES):
        ...
        if not check_sim_at(timeout=15):
            log_no_time("SIM7070G not responding, resetting...")
            power_on_sim()
            reset_sim7070()
