...
def log_no_time(message, log_path = "/home/gedonsoft/Weather/capture.log"):
    with open(log_path, "a") as f:
        f.write(f"{message}\n") 

def get_logs(lines=44):
    try:
        return subprocess.check_output(["tail",f"-n{lines}","/home/gedonsoft/Weather/capture.log"]).decode("utf-8",errors="ignore")
...
try:
    while True:
        ...
        SSID = str(wait_for_uart(uart)) 
        PW = str(wait_for_uart(uart))
        print (type(SSID))
        log_no_time(f"{SSID}, {PW}")

        if BatteryData is None:
            log_no_time("No response from ESP about Battery")
            break
        else:
            log_no_time(f"Battery: {BatteryData}! Safe Battery Levels are 12.6V - 9V")
            break
    uart.close()
    time.sleep(1)
    ...
    try:
        log_no_time(f"New {SSID}, {PW}")
        subprocess.run(["nmcli","radio","wifi","on"])
        subprocess.run(["nmcli", "dev", "wifi", "connect", str(SSID), "password", str(PW)])
        logging.info("Connecting with Wifi")
        sync_time_after_ppp()
    except Exception as e:
        logging.error(e)
    ...
    for attempt in range(1, MAX_RETRIES):
        try:
            ...
            with open('/home/gedonsoft/Weather/capture.log', 'rb') as k:
